 ...
 * 
 * **ケース1: カバレッジ100%（返金なし）**
 * - 保険コミットメント月額: $171.55 (固定・全10台分)
 * - オンデマンドカバー: $397.12 (10台×100%)
 * - 削減額: $397.12 - $171.55 = $225.57 (正の値)
 * - 返金: $0
 * - 保険料: $225.57 × 0.33 = $74.44
 * - 残りオンデマンド: $0.00
 * - 月額コスト: $171.55 + $74.44 + $0.00 - $0.00 = $245.99
 * 
 * **ケース2: カバレッジ50%（返金なし）**
 * - 保険コミットメント月額: $171.55 (固定・変わらず)
 * - オンデマンドカバー: $198.56 (5台分のみ)
 * - 削減額: $198.56 - $171.55 = $27.01 (正の値)
 * - 返金: $0
 * - 保険料: $27.01 × 0.33 = $8.91
 * - 残りオンデマンド: $198.56 (5台分)
 * - 月額コスト: $171.55 + $8.91 + $198.56 - $0.00 = $379.02
 * 
 * **ケース3: カバレッジ30%（返金発生）**
 * - 保険コミットメント月額: $171.55 (固定・変わらず)
 * - オンデマンドカバー: $119.14 (3台分のみ)
 * - 削減額: $119.14 - $171.55 = -$52.41 (マイナス)
 * - 返金: $52.41 (未使用分を返金)
 * - 保険料: $0 (返金発生時は保険料なし)
 * - 残りオンデマンド: $277.98 (7台分)
 * - 月額コスト: $171.55 + $0.00 + $277.98 - $52.41 = $397.12
 * 
 * **重要ポイント**:
 * - 保険コミットメント料金は常に$171.55で固定（カバレッジに無関係）
 * - カバレッジが下がると返金が発生し、実質コストが調整される
 * 
 * ### レベニュー計算:
 * ```
...
**主要な変更**:
1. **34a0c1c** - カバレッジ0%の正しい処理（`|| 1.0` → `!== undefined ? : 1.0`）
2. **3570e0f** - レベニュー予測レポートの範囲調整（0%: 0-20%, 100%: 80-100%）
3. **d81307c** - 保険コミットメントを固定料金に修正（カバレッジに無関係）
4. **8c6b23a** - 基準を3年AllUpfront→3年NoUpfrontに修正
5. **186fa91** - Reserved Capacity料金表追加（OpenSearch, Redshift, DynamoDB）
6. **cbff2c1** - 用語統一「保険RI/SP」→「保険コミットメント」
7. **947fa01** - レベニュー掛率を30%→20%に変更

**重要な修正内容**:
- **保険コミットメント料金**: 標準RI/SP 3年NoUpfrontの100%使用時コストを基準として固定
- **カバレッジの影響**: 保険料金には影響せず、オンデマンドとの比較にのみ影響
- **返金計算**: カバレッジが低い場合、未使用分が返金される
- **レベニュー**: 保険料の20%

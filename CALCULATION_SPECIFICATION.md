...
このシミュレーターは、AWS保険コミットメントと標準RI/SP（Reserved InstancesおよびSavings Plans）のコスト比較を行い、損益分岐点を算出するツールです。

### 主な機能

- 💰 保険コミットメントと標準RI/SPのコスト計算
- 📊 累積コストの可視化
- 🎯 損益分岐点の自動算出
- 📈 契約期間全体の総支出比較
...
|-------|-------|---------|-----------|---------|
| 30日保証 | 60% | 50% | **30.0%** | $55.60 |
| 1年保証 | 60% | 33% | **40.2%** | $47.50 |
...
実効割引率 = ($79.42 - $47.50) / $79.42 × 100 = 40.2%
```
...
保険コミットメント:
  1年保証（実効）:    40.2% ← 標準RI/SPと同等
  30日保証（実効）:   30.0%
```

**使い分けの指針**

1. **最大割引を求める**: RI AllUpfront (41.2%)
2. **初期費用を避けたい**: SP 1年 (40.0%) または保険コミットメント 1年保証 (40.2%)
3. **柔軟性を重視**: 保険コミットメント 1年保証 (40.2%) ← 高割引率＋短期契約＋返金保証
4. **超短期利用**: 保険コミットメント 30日保証 (30.0%)
...
```typescript
// 基本削減額
baseSavings = 通常価格月額 - 月額コスト

// 実効削減額（返金見込みを含む）
monthlySavings = baseSavings + expectedRefund
```
...
**重要**: 保険コミットメント自体の契約期間（30日）ではなく、**選択した標準RI/SPの契約期間**を使用します。これにより公平な比較が可能になります。
...
for (month = 1 to 契約期間月数) {
  ...
  cumulative.insurance.push(保険コミットメント月額 × month)
...
| 項目 | 説明 | 選択肢 | デフォルト |
|------|------|--------|-----------|
| 🛡️ 保険コミットメントプラン | 保険プランの選択 | 30日保証 (30%割引/50%保険料)<br>1年保証 (45%割引/33%保険料) | 1年保証 |
...
| リソース | 通常価格 | 保険コミットメント<br>月額 | 保険料 | 削減額 | 標準RI/SP<br>月額 | 初期費用 | 削減額 |
|---------|---------|-----------------|--------|--------|-----------------|---------|--------|
...
interface PlanResult {
  ...
  premium: number                   // 保険料（保険コミットメントのみ）
...
interface SimulationResult {
  ...
  insurance: PlanResult      // 保険コミットメント結果
...
interface CumulativeData {
  ...
  insurance: number[]    // 保険コミットメント累積配列
...
| 保険コミットメント（累積） | rgba(16, 185, 129, 1) | 実線 | 保険コミットメント累積コスト |
| 保険コミットメント（総支出） | rgba(16, 185, 129, 0.6) | 破線 | 保険コミットメント総支出（固定） |
...
| 保険コミットメント | $166.38 | $0 | $1,996.56 | 9ヶ月 |
...
### ケース3: 標準RI/SP契約期間による保険コミットメント損益分岐月の変化
...
```
保険コミットメント総支出 = $0 + ($166.38 × 12) = $1,996.56
損益分岐月 = ceil($1,996.56 / $238.25) = 9ヶ月
```
...
```
保険コミットメント総支出 = $0 + ($166.38 × 36) = $5,989.68
損益分岐月 = ceil($5,989.68 / $238.25) = 26ヶ月
```

**重要**: 保険コミットメントの損益分岐月は、**選択した標準RI/SPの契約期間**に依存します。これは公平な比較のための設計です。
...
3. **保険コミットメントは標準契約期間を使用**: 公平な比較のため、標準RI/SPの契約期間で総支出を計算
